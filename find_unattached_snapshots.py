...
def find_unattached_snapshots(ec2, snapshots):
    """Find snapshots not associated with any AMI using parallel processing."""
...
def save_data_to_csv(data, filename):
    ...
    with open(filename, mode='w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=['SnapshotId'])
        writer.writeheader()
        for row in data:
...
def generate_filename(profile, region):
    ...
    filename = f"unattached_snapshots_{profile}_{region}_{current_time}.csv"
    return filename
...
def main():
    ...
    try:
        ...
        unattached_snapshots = find_unattached_snapshots(ec2, snapshots)

        # Generate filename dynamically
        output_file = generate_filename(default_profile, default_region)

        # Save data to the generated file
        save_data_to_csv(unattached_snapshots, output_file)
    except Exception as e:
